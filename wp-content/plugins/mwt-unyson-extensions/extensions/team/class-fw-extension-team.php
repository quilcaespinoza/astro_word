...
class FW_Extension_Team extends FW_Extension {
	...
	public function _action_add_permalink_in_settings() {
		add_settings_field(
			...
			__( 'Team member base', 'mwt-unyson-extensions' ),
		...
		add_settings_field(
			...
			__( 'Team category base', 'mwt-unyson-extensions' ),
	...
	public function _action_register_post_type() {
		...
		$post_names = apply_filters( 'fw_ext_team_post_type_name',
			array(
				'singular' => esc_html__( 'Team member', 'mwt-unyson-extensions' ),
				'plural'   => esc_html__( 'Team member', 'mwt-unyson-extensions' )
			) );

		register_post_type( $this->post_type,
			array(
				'labels'             => array(
					'name'               => $post_names['plural'], //__( 'Team', 'mwt-unyson-extensions' ),
					'singular_name'      => $post_names['singular'], //__( 'Team member', 'mwt-unyson-extensions' ),
					'add_new'            => esc_html__( 'Add New', 'mwt-unyson-extensions' ),
					'add_new_item'       => sprintf( esc_html__( 'Add New %s', 'mwt-unyson-extensions' ), $post_names['singular'] ),
					'edit'               => esc_html__( 'Edit', 'mwt-unyson-extensions' ),
					'edit_item'          => sprintf( esc_html__( 'Edit %s', 'mwt-unyson-extensions' ), $post_names['singular'] ),
					'new_item'           => sprintf( esc_html__( 'New %s', 'mwt-unyson-extensions' ), $post_names['singular'] ),
					'all_items'          => sprintf( esc_html__( 'All %s', 'mwt-unyson-extensions' ), $post_names['plural'] ),
					'view'               => sprintf( esc_html__( 'View %s', 'mwt-unyson-extensions' ), $post_names['singular'] ),
					'view_item'          => sprintf( esc_html__( 'View %s', 'mwt-unyson-extensions' ), $post_names['singular'] ),
					'search_items'       => sprintf( esc_html__( 'Search %s', 'mwt-unyson-extensions' ), $post_names['plural'] ),
					'not_found'          => sprintf( esc_html__( 'No %s Found', 'mwt-unyson-extensions' ), $post_names['plural'] ),
					'not_found_in_trash' => sprintf( esc_html__( 'No %s Found In Trash', 'mwt-unyson-extensions' ), $post_names['plural'] ),
					'parent_item_colon'  => '' /* text for parent types */
				),
				'description'        => esc_html__( 'Create a team item', 'mwt-unyson-extensions' ),
				'public'             => true,
				'show_ui'            => true,
				'show_in_menu'       => true,
	...
	public function _action_register_taxonomy() {
		...
		$category_names = apply_filters( 'fw_ext_team_category_name',
			array(
				'singular' => esc_html__( 'Category', 'mwt-unyson-extensions' ),
				'plural'   => esc_html__( 'Categories', 'mwt-unyson-extensions' )
			) );

		$labels = array(
			'name'              => sprintf( _x( 'Team %s', 'taxonomy general name', 'mwt-unyson-extensions' ),
				$category_names['plural'] ),
			'singular_name'     => sprintf( _x( 'Team %s', 'taxonomy singular name', 'mwt-unyson-extensions' ),
				$category_names['singular'] ),
			'search_items'      => sprintf( esc_html__( 'Search %s', 'mwt-unyson-extensions' ), $category_names['plural'] ),
			'all_items'         => sprintf( esc_html__( 'All %s', 'mwt-unyson-extensions' ), $category_names['plural'] ),
			'parent_item'       => sprintf( esc_html__( 'Parent %s', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'parent_item_colon' => sprintf( esc_html__( 'Parent %s:', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'edit_item'         => sprintf( esc_html__( 'Edit %s', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'update_item'       => sprintf( esc_html__( 'Update %s', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'add_new_item'      => sprintf( esc_html__( 'Add New %s', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'new_item_name'     => sprintf( esc_html__( 'New %s Name', 'mwt-unyson-extensions' ), $category_names['singular'] ),
			'menu_name'         => sprintf( esc_html__( '%s', 'mwt-unyson-extensions' ), $category_names['plural'] )
		);
		$args   = array(
			'labels'            => $labels,
			'public'            => true,
			...
			if ( $post_type === $this->post_type ) {
				...
				$options[] = array(
					'member_meta_tab' => array(
						'title'   => esc_html__( 'Member Meta', 'mwt-unyson-extensions' ),
						'type'    => 'box',
						'options' => array(
							'tab_info' => array(
								'title'   => esc_html__( 'Member Info list', 'mwt-unyson-extensions' ),
								'type'    => 'tab',
								'options' => array(
									'member_position'       => array(
										'type'  => 'text',
										'label' => esc_html__('Position', 'canabro'),
									),
									$member_list_icons
								),
							),
							'tab_social_icons' => array(
								'title'   => esc_html__( 'Social links', 'mwt-unyson-extensions' ),
								'type'    => 'tab',
								'options' => array(
									$member_social_icons,
								),
							...
							'tab_skills' => array(
								'title'   => esc_html__( 'Member Skills', 'mwt-unyson-extensions' ),
								'type'    => 'tab',
								'options' => array(
									'skills' => array(
										'type'            => 'addable-box',
										'value'           => '',
										'label'           => esc_html__( 'Skills progress bars', 'mwt-unyson-extensions' ),
										'desc'            => esc_html__( 'Display skills as a progress bar', 'mwt-unyson-extensions' ),
										'template'        => '{{=title}}',
										'box-options'     => array(
											$member_skills,
										),
							...
							'tab_form' => array(
								'title'   => esc_html__( 'Member Contact Form', 'mwt-unyson-extensions' ),
								'type'    => 'tab',
								'options' => array(
									$contact_form,
								),
	...
	public function _action_admin_rename_team() {
		...
		foreach ( $menu as $key => $menu_item ) {
			if ( $menu_item[2] == 'edit.php?post_type=' . $this->post_type ) {
				$menu[ $key ][0] = esc_html__( 'Team', 'mwt-unyson-extensions' );
			}
		}
	}
	...
	public function _action_admin_featured_image_label() {
		...
		add_meta_box(
			'postimagediv',
			__( 'Team Member Photo', 'mwt-unyson-extensions' ),
			...
			case 'image':
				if ( get_the_post_thumbnail( intval( $id ) ) ) {
					$value = '<a href="' . get_edit_post_link( $id,
							true ) . '" title="' . esc_attr( esc_html__( 'Edit this item', 'mwt-unyson-extensions' ) ) . '">' .
	...
	public function _action_admin_add_team_edit_page_filter() {
		...
		if ( empty( $terms ) || is_wp_error( $terms ) ) {
			echo '<select name="' . $this->get_name() . '-filter-by-team-category"><option value="0">' . esc_html__( 'View all categories',
					'mwt-unyson-extensions' ) . '</option></select>';
		...
		$dropdown_options = array(
			...
			'show_option_all' => esc_html__( 'View all categories', 'mwt-unyson-extensions' ),
			'hide_empty'      => true,
			'hierarchical'    => 1,
			'show_count'      => 0,
			'orderby'         => 'name',
	...
	public function _filter_admin_manage_edit_columns( $columns ) {
		...
		$new_columns['image'] = esc_html__( 'Feat. Image', 'mwt-unyson-extensions' );
