...
class FW_Extension_Services extends FW_Extension {
	...
	public function _action_add_permalink_in_settings() {
		add_settings_field(
			...
			__( 'Service base', 'mwt-unyson-extensions' ),
		...
		add_settings_field(
			...
			__( 'Services category base', 'mwt-unyson-extensions' ),
	...
	public function _action_register_post_type() {
		...
		$post_names = apply_filters( 'fw_ext_services_post_type_name',
			array(
				'singular' => esc_html__( 'Service', 'mwt-unyson-extensions' ),
				'plural'   => esc_html__( 'Services', 'mwt-unyson-extensions' )
			) );

		register_post_type( $this->post_type,
			array(
				'labels'             => array(
					'name'               => $post_names['plural'], //__( 'Services', 'mwt-unyson-extensions' ),
					'singular_name'      => $post_names['singular'], //__( 'Services service', 'mwt-unyson-extensions' ),
				...
				'description'        => esc_html__( 'Create a services item', 'mwt-unyson-extensions' ),
				'public'             => true,
				'show_ui'            => true,
				'show_in_menu'       => true,
	...
	public function _action_register_taxonomy() {
		...
		$labels = array(
			'name'              => sprintf( _x( 'Services %s', 'taxonomy general name', 'mwt-unyson-extensions' ),
				$category_names['plural'] ),
			'singular_name'     => sprintf( _x( 'Services %s', 'taxonomy singular name', 'mwt-unyson-extensions' ),
	...
	public function _filter_admin_add_post_options( $options, $post_type ) {
		...
		if ( !empty( $shortcodes_extension ) ) {
			$progress_bar = $shortcodes_extension->get_shortcode( 'progress_bar' )->get_options();
			...
			if ( $post_type === $this->post_type ) {
				...
				$options[] = array(
					'icon_box' => array(
						'context' => 'side',
						'title'   => esc_html__( 'Service', 'mwt-unyson-extensions' ) . ' ' . esc_html__( 'Icon', 'mwt-unyson-extensions' ),
						'type'    => 'box',
						'options' => array(
					...
					'service_experience' => array(
						'context' => 'normal',
						'title'   => esc_html__( 'Service Progress Bars', 'mwt-unyson-extensions' ),
						'type'    => 'box',
						'options' => array(
							'title'       => array(
								'type' => 'text',
							...
							'progress_bars' => array(
								'type'            => 'addable-box',
								'value'           => '',
								'label'           => esc_html__( 'Progress Bars', 'mwt-unyson-extensions' ),
								'template'        => '{{=title}}',
								'box-options'     => array(
									$progress_bar,
								),
	...
	public function _action_admin_rename_services() {
		...
		foreach ( $menu as $key => $menu_item ) {
			if ( $menu_item[2] == 'edit.php?post_type=' . $this->post_type ) {
				$menu[ $key ][0] = esc_html__( 'Services', 'mwt-unyson-extensions' );
			}
		}
	}
	...
	public function _action_admin_featured_image_label() {
		...
		add_meta_box(
			'postimagediv',
			__( 'Single Service Image', 'mwt-unyson-extensions' ),
	...
	public function _action_admin_add_services_edit_page_filter() {
		...
		if ( empty( $terms ) || is_wp_error( $terms ) ) {
			echo '<select name="' . $this->get_name() . '-filter-by-services-category"><option value="0">' . esc_html__( 'View all categories',
