...
if ( ! function_exists( 'mwt_print_post_likes' ) ) :
	 ...
	 */
	function mwt_print_post_likes( $count, $only_number = false ) {
		$html = '';
		if ( !is_single() || $only_number ) {
			$html = '<span class="item-likes-count">' . $count . '</span>';
		} else {
			if ( ! $count ) {
...
if ( ! function_exists( 'mwt_post_like_button' ) ) :
	...
	function mwt_post_like_button( $postID ) {
		...
		if ( empty( $_COOKIE["$postID"] ) ) {
			...
			$output = '<span data-id="' . $postID . '"><span class="like_button"><i class="fa fa-check highlight3"></i></span></span>';
		}
...
if ( ! function_exists( 'mwt_post_like_count' ) ) :
	 ...
	 */
	function mwt_post_like_count( $postID, $only_number = false ) {
		echo apply_filters( 'mwt_like_count', '<span class="votes_count votes_count_' . $postID . '">' . mwt_print_post_likes( mwt_get_post_likes( $postID ), $only_number ) . '</span>' );
	} //mwt_post_like_count()
endif;
