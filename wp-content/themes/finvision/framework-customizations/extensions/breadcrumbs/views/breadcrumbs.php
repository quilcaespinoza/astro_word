	...
	<ol class="breadcrumb">
		<?php for ( $i = 0; $i < count( $items ); $i ++ ) : ?>
			<?php if ( $i == ( count( $items ) - 1 ) ) : ?>
                <li class="last-item"><span><?php echo wp_kses_post( $items[ $i ]['name'] ); ?></span></li>
			<?php elseif ( $i == 0 ) : ?>
				<li class="first-item">
				<?php if ( isset( $items[ $i ]['url'] ) ) : ?>
					<a href="<?php echo esc_url( wp_kses_post( $items[ $i ]['url'] ) ); ?>"><?php echo wp_kses_post( $items[ $i ]['name'] ); ?></a></li>
