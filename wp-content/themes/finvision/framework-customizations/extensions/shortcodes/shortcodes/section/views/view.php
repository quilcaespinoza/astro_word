...
$section_extra_classes .= $atts['skew_overlay']['is_overlay'] === 'overlay' ? ' with_skew_overlay' : '';
...
$unique_id = uniqid();
?>
<section class="fw-main-row <?php echo esc_attr( $section_extra_classes ) ?>" <?php if ( $bg_image ) { echo 'style="' . esc_attr( $bg_image ) . '"'; } ?><?php if ( !empty( $section_id )  ) { echo ' id="' . esc_attr( $section_id ) . '"'; } ?>>

    <?php if ( $atts['skew_overlay']['is_overlay'] ) : ?>
        <div class="skew-overlay <?php echo esc_attr( $atts['skew_overlay']['overlay']['overlay_side'] ); ?>"></div>
    <?php endif; ?>
	...
	?>
		<div class="image_cover <?php echo ( ! empty( $atts['side_media_position'] ) ) ? esc_attr( $atts['side_media_position'] ) : '' ; ?>">
			<?php if ( $link ): ?>
			<a href="<?php echo esc_url( $link ); ?>" <?php if ( $video ) { echo ' data-gal="prettyPhoto[gal-video-'. $unique_id .']"'; } ?>></a>
			<?php endif; //$link ?>
			<img src="<?php echo esc_attr($atts['side_media_image']['url'] )?>">
		</div>
	<?php
		endif;
	?>
