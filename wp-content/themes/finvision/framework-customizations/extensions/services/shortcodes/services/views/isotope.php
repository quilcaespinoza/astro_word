...
//bootstrap col-xs-* class
$xxs_class = '';
switch ( $atts['responsive_xxs'] ) :
	case ( 1 ) :
		$xxs_class = 'col-xxs-12';
		break;
	default:
		$xxs_class = '';
endswitch;
...
switch ( $atts['margin'] ) :
	...
	case ( 60 ) :
		$margin_class = 'columns_padding_30';
		break;
	//6
	default:
		$margin_class = '';
...
<div class="isotope_container services isotope row <?php echo esc_attr( $margin_class ); ?> masonry-layout <?php if ( $margin_class === 'columns_padding_30' ) { echo 'columns_margin_bottom_50'; } ?>"
...
>
    <?php while ( $posts->have_posts() ) : $posts->the_post(); ?>
        <div
            class="isotope-item <?php echo esc_attr( $lg_class . ' ' . $md_class . ' ' . $sm_class . ' ' . $xs_class . ' ' . $xxs_class . ' ' . $sort_classes[get_the_ID()] ); ?>">
            <?php
                include( fw()->extensions->get( 'services' )->locate_view_path( 'loop-' . $item_layout ) );
            ?>
        </div>
    <?php endwhile; ?>
