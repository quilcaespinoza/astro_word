...
if (!function_exists('finvision_action_in_widget_form') ) :
	function finvision_action_in_widget_form( $t, $return, $instance ) {
        ...
        <?php if ( $t->id_base === 'search' || $t->id_base === 'mc4wp_form_widget' ) : ?>
            <p class="custom_text_option">
                <label
                        for="<?php echo esc_attr( $t->get_field_id( 'custom_text' ) ); ?>"><?php esc_html_e( 'Custom text that appear under title:', 'finvision' ); ?>
                </label><br>
		...
		<p class="widget_background_option">
			<label for="<?php echo esc_attr( $t->get_field_id( 'widget_background' ) ); ?>"><?php esc_html_e( 'Widget Background:', 'finvision' ); ?></label>
			<select id="<?php echo esc_attr( $t->get_field_id( 'widget_background' ) ); ?>"
				...
				<option <?php selected( $instance['widget_background'], '' ); ?>
					value=""><?php esc_html_e( 'None', 'finvision' ); ?></option>
				<option
					<?php selected( $instance['widget_background'], 'with_background with_padding' ); ?>value="with_background with_padding"><?php esc_html_e( 'Muted Backgorund', 'finvision' ); ?></option>
                <option
					<?php selected( $instance['widget_background'], 'with_border with_padding' ); ?>value="with_border with_padding"><?php esc_html_e( 'Transparent With Border', 'finvision' ); ?></option>
                <option
					<?php selected( $instance['widget_background'], 'with_background with_border with_padding' ); ?>value="with_background with_border with_padding"><?php esc_html_e( 'Muted Backgorund With Border', 'finvision' ); ?></option>
                <option <?php selected( $instance['widget_background'], 'ls with_padding' ); ?>
                        value="ls with_padding"><?php esc_html_e( 'Light Background', 'finvision' ); ?></option>
				<option <?php selected( $instance['widget_background'], 'ds gradient lightened with_padding' ); ?>
					value="ds gradient lightened with_padding"><?php esc_html_e( 'Dark Background', 'finvision' ); ?></option>
                <option <?php selected( $instance['widget_background'], 'cs gradient lightened with_padding' ); ?>
                        value="cs gradient lightened with_padding"><?php esc_html_e( 'Accent Color Background', 'finvision' ); ?></option>
			</select>
		</p>
		<p class="widget_bootstrap_width">
			<label
				for="<?php echo esc_attr( $t->get_field_id( 'bootstrap_width' ) ); ?>"><?php esc_html_e( 'Widget Column Width:', 'finvision' ); ?>
			</label>
			<select id="<?php echo esc_attr( $t->get_field_id( 'bootstrap_width' ) ); ?>"
				...
				<option <?php selected( $instance['bootstrap_width'], '' ); ?> value=""><?php esc_html_e( 'None', 'finvision' ); ?></option>
				<option <?php selected( $instance['bootstrap_width'], 'col-md-3 col-sm-6 col-xs-12' ); ?>value="col-md-3 col-sm-6 col-xs-12">1/4</option>
				<option <?php selected( $instance['bootstrap_width'], 'col-md-4 col-sm-6 col-xs-12' ); ?> value="col-md-4 col-sm-6 col-xs-12">1/3</option>
				<option <?php selected( $instance['bootstrap_width'], 'col-sm-6 col-xs-12' ); ?> value="col-sm-6 col-xs-12">1/2</option>
				<option <?php selected( $instance['bootstrap_width'], 'col-xs-12' ); ?>value="col-xs-12"><?php esc_html_e( 'Full Width', 'finvision' ); ?></option>
                <option <?php selected( $instance['bootstrap_width'], 'previous-column' ); ?>value="previous-column"><?php esc_html_e( 'Put in previous widget column', 'finvision' ); ?></option>
			</select>
		</p>
        <p class="widget_bootstrap_custom_width">
            <label
                    for="<?php echo esc_attr( $t->get_field_id( 'bootstrap_custom_width' ) ); ?>"><?php esc_html_e( 'Custom Column Width:', 'finvision' ); ?>
            </label>
            ...
            <?php echo esc_html__( 'Use bootstrap grid classes or left it empty.', 'finvision' ); ?>
        </p>
        <p class="widget_text_align">
            <label
                    for="<?php echo esc_attr( $t->get_field_id( 'text_align' ) ); ?>"><?php esc_html_e( 'Widget Text Align:', 'finvision' ); ?>
            </label>
            <select id="<?php echo esc_attr( $t->get_field_id( 'text_align' ) ); ?>"
                    name="<?php echo esc_attr( $t->get_field_name( 'text_align' ) ); ?>" class="widefat">
                <option <?php selected( $instance['text_align'], '' ); ?> value=""><?php esc_html_e( 'Left', 'finvision' ); ?></option>
                <option <?php selected( $instance['text_align'], 'text-center' ); ?>value="text-center"><?php esc_html_e( 'Center', 'finvision' ); ?></option>
                <option <?php selected( $instance['text_align'], 'text-right' ); ?> value="text-right"><?php esc_html_e( 'Right', 'finvision' ); ?></option>
            </select>
        </p>
	...
	} //finvision_action_in_widget_form()
endif;

if( !function_exists('finvision_filter_in_widget_form_update') ) :
	function finvision_filter_in_widget_form_update( $instance, $new_instance, $old_instance ) {
		...
		return $instance;
	} //finvision_filter_in_widget_form_update()
endif;

if( !function_exists( 'finvision_filter_dynamic_sidebar_params' ) ):
	function finvision_filter_dynamic_sidebar_params( $params ) {
		...
		return $params;
	} //finvision_filter_dynamic_sidebar_params()
endif;

//Add input fields(priority 5, 3 parameters)
add_action( 'in_widget_form', 'finvision_action_in_widget_form', 5, 3 );
//Callback function for options update (priority 5, 3 parameters)
add_filter( 'widget_update_callback', 'finvision_filter_in_widget_form_update', 5, 3 );
//add class names (default priority, one parameter)
add_filter( 'dynamic_sidebar_params', 'finvision_filter_dynamic_sidebar_params', 1 );
