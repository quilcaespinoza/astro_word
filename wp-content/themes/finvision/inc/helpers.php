...
if ( ! function_exists( 'finvision_google_font_url' ) ) :
	function finvision_google_font_url() {
		$fonts_url = '';
		$fonts     = array();
		...
		$fonts['Montserrat'] = array(
			...
			'subset'         => 'latin',
			'variation'      => '400',
			'variants'       => array( '300', '400', '400i', '500', '600', '700' ),
			'family'         => 'Montserrat',
			'style'          => false,
			'weight'         => false,
			'size'           => '18',
...
if ( ! function_exists( 'finvision_add_font_styles_in_head' ) ) :
	function finvision_add_font_styles_in_head() {
	...
	} //finvision_add_font_styles_in_head()
...
if ( ! function_exists( 'finvision_return_comments_walker' ) ) :
	function finvision_return_comments_walker() {
		return new Finvision_Comments_Walker;
	}
endif;


if ( ! function_exists( 'finvision_the_attached_image' ) ) :
	/**
	 * Print the attached image with a link to the next attached image.
	 */
	function finvision_the_attached_image() {
		$post = get_post();
		/**
		 ...
		 * }
		 */
		$attachment_size     = apply_filters( 'finvision_attachment_size', array( 810, 810 ) );
	...
	} //finvision_the_attached_image()

endif;

if ( ! function_exists( 'finvision_list_authors' ) ) :
	/**
	 * Print a list of all site authors who published at least one post.
	 */
	function finvision_list_authors($only_post_author = true) {
		if ( $only_post_author ) {
		...
		foreach ( $author_ids as $author_id ) :
			...
			?>
			<div class="author-meta side-item side-sm no-content-padding">
			    <h2><?php echo esc_html__( 'Author', 'finvision' ); ?></h2>
				<div class="row">
					...
					<div class="col-xs-12 col-sm-8">
						<div class="item-content">
                            <header class="entry-header">
                                <div class="entry-meta inline-content with_dividers grey dakrlinks">
                                    <h4><?php the_author_posts_link(); ?></h4>
                                    <?php if ( !empty( $author_position ) ) : ?>
                                        <span><?php echo wp_kses_post( $author_position ); ?></span>
                                    <?php endif; ?>
                                </div>
                            </header>
                            <?php if ( $author_bio ) : ?>
							...
							if ( $twitter_url || $facebook_url || $google_plus_url || $youtube ) :
								?>
								<span class="author-social">
									<?php if ( $twitter_url ) : ?>
                                         <a href="<?php echo esc_url( $twitter_url ) ?>"
                                               class="social-icon color-icon border-icon rounded-icon socicon-twitter"></a>
									<?php endif; ?>
									<?php if ( $facebook_url ) : ?>
                                         <a href="<?php echo esc_url( $facebook_url ) ?>"
                                               class="social-icon color-icon border-icon rounded-icon socicon-facebook"></a>
									<?php endif; ?>
									<?php if ( $google_plus_url ) : ?>
                                         <a href="<?php echo esc_url( $google_plus_url ) ?>"
                                               class="social-icon color-icon border-icon rounded-icon socicon-googleplus"></a>
									<?php endif; ?>
									<?php if ( $youtube ) : ?>
                                         <a href="<?php echo esc_url( $youtube ) ?>"
                                               class="social-icon color-icon border-icon rounded-icon socicon-youtube"></a>
									<?php endif; ?>
...
if ( ! function_exists( 'finvision_is_active_widgets_in_main_sidebar_exists' ) ) :
	/**
	 * Define is sidebar that must be shown has active widgets
	 */
	function finvision_is_active_widgets_in_main_sidebar_exists() {
		//default value
		$return = true;
...
if ( ! function_exists( 'finvision_get_columns_classes' ) ) :
	/**
	 * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
	 * return array
	 */
	function finvision_get_columns_classes( $full_width = false ) {
		//default values
		$column_classes = array(
			'main_column_class' => 'col-xs-12 col-md-8 col-lg-8',
			'sidebar_class'     => 'col-xs-12 col-sm-6 col-sm-offset-3 col-md-4 col-md-offset-0 col-lg-4'
		);
		if ( is_page() ) {
		...
		if ( function_exists( 'fw_ext_sidebars_get_current_position' ) ) {
			...
			if ( in_array( fw_ext_sidebars_get_current_position(), array( 'full' ) ) ) {
				...
				$column_classes['main_column_class'] = "col-xs-12 col-md-8 col-lg-8 col-md-push-4 col-lg-push-4";
				$column_classes['sidebar_class']     = "col-xs-12 col-sm-6 col-md-4 col-lg-4 col-md-pull-8 col-lg-pull-8 col-sm-offset-3 col-md-offset-0";

			} elseif ( in_array( fw_ext_sidebars_get_current_position(), array( 'right' ) ) ) {

				$column_classes['main_column_class'] = "col-xs-12 col-md-8 col-lg-8";
				$column_classes['sidebar_class']     = "col-xs-12 col-sm-6 col-sm-offset-3 col-md-4 col-md-offset-0 col-lg-4";
		...
		if ( $full_width || !finvision_is_active_widgets_in_main_sidebar_exists() ) {
			$column_classes['main_column_class'] = "col-xs-12";
			$column_classes['sidebar_class']     = false;
		}
	...
	} //finvision_get_columns_classes()

endif;

if ( ! function_exists( 'finvision_get_columns_classes_for_unyson_extended' ) ) :
	/**
	 * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
	 * return array
	 */
	function finvision_get_columns_classes_for_unyson_extended( $full_width = false ) {

		// Sidebar Position

		// default
		$sidebar_position = apply_filters( 'finvision_default_sidebar_position', 'right' );
		...
		$empty = false;
		if ( apply_filters( 'finvision_check_for_widgets', true ) ) {
		...
		$s = apply_filters( 'finvision_sidebar_width', 4 );             // sidebar width
 ...
 */
if ( ! function_exists( 'finvision_bootstrap_paginate_links' ) ) {
	function finvision_bootstrap_paginate_links( $args = '' ) {
		...
		$defaults = array(
			...
			'show_all'           => false,
			'prev_next'          => true,
			'prev_text'          => '<span class="sr-only">' . esc_html__( 'Prev', 'finvision' ) . '</span><i class="fa fa-angle-left"></i>',
			'next_text'          => '<span class="sr-only">' . esc_html__( 'Next', 'finvision' ) . '</span><i class="fa fa-angle-right"></i>',
			'end_size'           => 1,
			'mid_size'           => 2,
			'type'               => 'plain',
			'add_args'           => array(),
		...
		return $r;
	} //finvision_bootstrap_paginate_links()
}

if ( ! function_exists( 'finvision_paging_nav' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function finvision_paging_nav( $wp_query = null, $wrapper = null ) {
		...
		$links = finvision_bootstrap_paginate_links( array(
			'base'      => $pagenum_link,
			'format'    => $format,
			'show_all'  => false,
...
if ( ! function_exists( 'finvision_paging_comments_nav ' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function finvision_paging_comments_nav( $args = array() ) {
		...
		$args       = wp_parse_args( $args, $defaults );
		$page_links = finvision_bootstrap_paginate_links( $args );
 ...
 */
if ( ! function_exists( 'finvision_categorized_blog' ) ) :
	function finvision_categorized_blog() {
		if ( false === ( $all_categories = get_transient( 'finvision_category_count' ) ) ) {
			...
			set_transient( 'finvision_category_count', $all_categories );
		}

		if ( 1 !== (int) $all_categories ) {
			// This blog has more than 1 category so finvision_categorized_blog should return true
			return true;
		} else {
			// This blog has only 1 category so finvision_categorized_blog should return false
			return false;
		}
	} //finvision_categorized_blog()
endif;

if ( ! function_exists( 'finvision_post_nav' ) ) :
	/**
	 * Display navigation to next/previous post when applicable.
	 */
	function finvision_post_nav() {
            ...
            <div class="col-xs-12 col-sm-6">
				<?php if ( $previous ) : ?>
                    <div class="nav-item prev-item">
                        <a href="<?php echo esc_url( get_permalink( $previous ) ); ?>" class="theme_button block_button"><?php echo esc_html__( 'Previous Post', 'finvision' ); ?></a>
                    </div>
				<?php endif; ?>
            </div>
            <div class="col-xs-12 col-sm-6 text-center">
                <?php if ( $next ) : ?>
                    <div class="nav-item next-item">
                        <a href="<?php echo esc_url( get_permalink( $next ) ); ?>" class="theme_button block_button"><?php echo esc_html__( 'Next Post', 'finvision' ); ?></a>
                    </div>
				<?php endif; ?>
            </div>
        </div>

	<?php } //finvision_post_nav
endif;

if ( ! function_exists( 'finvision_related_posts' ) ) :
	/**
	 * Display related posts.
	 */
	function finvision_related_posts( $layout = 'grid' ) {
		global $post;
		$orig_post = $post;
        $tags = wp_get_post_tags($post->ID);
        $hasSidebar = ( finvision_get_columns_classes()['main_column_class'] === 'col-xs-12' ) ? false : true;
        ...
        if ($tags) {
	        ...
	        $args=array(
		        ...
		        'posts_per_page'=> $post_per_page, // Number of related posts to display.
	        );
	        ...
	        if ( !empty( $my_query->posts ) ) { ?>
                ...
                <div class="post-related">
                    <h2>
                        <?php echo esc_html__( 'Related Posts', 'finvision' ); ?>
                    </h2>
                    ...
                    <?php while( $my_query->have_posts() ) {
                        ...
                        ?>
                            ...
                            <?php if ( $layout === 'grid' ) : ?>
                                <div class="isotope-item <?php echo esc_attr( $columns_class ); ?>">
                            <?php endif; ?>

                            <article id="post-<?php the_ID(); ?>" <?php post_class( 'vertical-item content-padding with_background' ); ?>>
 ...
 endif; //finvision_related_posts

if ( ! function_exists( 'finvision_posted_on' ) ) : /**
 * Print HTML with meta information for the current post-date/time and author.
 */
	function finvision_posted_on( $short = false, $linkcolor = "" ) {
		if ( ! $short ) {
			...
			printf( '<span class="entry-date"><a href="%5$s" rel="bookmark"><time class="entry-date" datetime="%6$s">%7$s</time></a></span>, <span class="author vcard"><a class="url fn n" href="%2$s" rel="author">%1$s %3$s</a></span>,',
				esc_html_x( 'by ', 'Used before post author name.', 'finvision' ),
				esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				get_the_author(),
				esc_html_x( 'on ', 'Used before post date.', 'finvision' ),
 ...
 */
if ( ! function_exists( 'finvision_post_thumbnail' ) ) :

	function finvision_post_thumbnail( $small_width_image = false, $square_thumb = false, $col_class = '"col-xs-12 col-sm-5 col-lg-5' ) {
		$pID = get_the_ID();
		$columns_classes = finvision_get_columns_classes();
        $blog_layout = 'regular';
        if ( function_exists( 'fw_get_db_customizer_option' ) ) {
		...
		if ( ( ! is_single() && $small_width_image ) || ( get_post_type() === 'fw-event' && $small_width_image ) ) :
			?>
	...
	<div class="item-media-wrap entry-thumbnail">
		<div class="item-media ds post-thumbnail<?php if ( !$is_gallery && !$iframe ) echo ' inline-block' ?>">
			<?php
			//featured image only for post
			if ( !$is_gallery || ( $is_gallery && empty( $galleries_image_ids ) ) ) :

                if ( $iframe ):
                    if ( $small_width_image && !$square_thumb ) : ?>
					    <div class="embed-responsive embed-responsive-3by4">
                    <?php elseif ( $square_thumb ): ?>
				...
				if ( $small_width_image && !$square_thumb ) {
					the_post_thumbnail( 'finvision-small-width' );
				} else if ( $square_thumb ) {
				    the_post_thumbnail( 'finvision-square-width' );
				} else if ( $columns_classes['main_column_class'] === 'col-xs-12'
                    && !( is_home() && $blog_layout === 'grid' )
                ) {
                    the_post_thumbnail( 'finvision-big-thumbnail-width' );
				} else {
					the_post_thumbnail();
				} //$current_position
                ...
                if ( $iframe ):
					if ( has_post_thumbnail() ): ?>
                        <div class="media-links">
						    <a href="<?php echo esc_url( $embed_url ); ?>" data-gal="prettyPhoto[gal-video]" class="embed-placeholder theme_button color1 round_button play_button margin_0"></a>
                        </div>
					<?php else:
			...
			// gallery with not empty $galleries_image_ids
			else :
				//featured image url
				...
				>
					...
					if ( $post_featured_image_src ) : ?>
						<div class="item">
                            <?php
                            if ( $square_thumb || $small_width_image ) {
                                echo wp_get_attachment_image( $post_thumbnail_id, 'finvision-square-width' );
                            } else if ( $columns_classes['main_column_class'] === 'col-xs-12' && !( is_home() && $blog_layout === 'grid' ) ) {
                                echo wp_get_attachment_image( $post_thumbnail_id, 'finvision-big-thumbnail-width' );
                            } else {
                    ...
                    foreach ( $galleries_image_ids as $image_id ) :
                            ...
                            <div class="item">
                                <?php
                                if ( $square_thumb || $small_width_image ) {
                                    echo wp_get_attachment_image( $image_id, 'finvision-square-width' );
                                } else if ( $columns_classes['main_column_class'] === 'col-xs-12' && !( is_home() && $blog_layout === 'grid' ) ) {
                                    echo wp_get_attachment_image( $image_id, 'finvision-big-thumbnail-width' );
                                } else {
			...
			<?php if ( finvision_show_post_date_in_thumbnail() ) : ?>
        ...
        <?php if ( has_tag('new') ) : ?>
            <div class="corner-label new"><?php echo esc_html__( 'new', 'finvision' ) ?></div>
        <?php endif; ?>
...
//check show post date in thumbnail or in post meta
if ( !function_exists( 'finvision_show_post_date_in_thumbnail') ) {
    function finvision_show_post_date_in_thumbnail() {
        $pID = get_the_ID();
        if (
            get_post_type() === 'post'
            && get_post_format() !== 'status'
            && get_post_format() !== 'quote'
            && ( has_post_thumbnail() || !empty( get_post_gallery( $pID, false )['ids'] ) )
            && ( get_option('date_format') === 'j M' || get_option('date_format') === 'j M Y' )
        ) {
            return true;
        } else {
            return false;
...
if ( ! function_exists( 'finvision_show_post_date_in_content' ) ) :
    function finvision_show_post_date_in_content() {
        $pID = get_the_ID();
        //detecting featured video
	    $embed_url = function_exists( 'fw_get_db_post_option' ) ? fw_get_db_post_option( $pID, 'post-featured-video', '' ) : '';
	    ...
	    return $show_date;
    } //finvision_show_post_date_in_content()
endif;

// share buttons
if ( ! function_exists( 'finvision_share_this' ) ) :
	/**
	 * Share article through social networks.
	 * bool $only_buttons
	 */
	function finvision_share_this( $only_buttons = false, $share_word = '', $icon_style = 'color-bg-icon' ) {
		if ( function_exists( 'mwt_share_this' ) ) {
			mwt_share_this( $only_buttons, $share_word, $icon_style );
		}

	} //finvision_share_this()
endif; //function_exists

//get predefined template part from theme options
if ( ! function_exists( 'finvision_get_predefined_template_part' ) ) :
	/**
	 * Return propper template part from options or default.
	 * string $template_part_name
	 */
	function finvision_get_predefined_template_part( $template_part_name ) {
	...
	} //finvision_get_predefined_template_part()

endif;//function_exists

//get ids of showing widgets
if ( ! function_exists( 'finvision_get_showing_widgets_ids' ) ) :
	/**
	 * Return array of id's of all widgets that are showing.
	 */

	function finvision_get_showing_widgets_ids() {
...
if ( ! function_exists( 'finvision_get_posts_single_taxonomy_name' ) ) :
	function finvision_get_posts_single_taxonomy_name() {
...
if ( ! function_exists( 'finvision_get_post_categories' ) ) :
	function finvision_get_post_categories( $taxonomy_name = 'category' ) {
...
if ( ! function_exists( 'finvision_get_categories_slugs_for_single_post' ) ) :
	function finvision_get_categories_slugs_for_single_post( $taxonomy_name = 'category' ) {
...
if ( ! function_exists( 'finvision_get_excerpt_for_page_with_unyson_builder' ) ) :
	function finvision_get_excerpt_for_page_with_unyson_builder() {
...
//post meta
if ( ! function_exists( 'finvision_the_post_meta' ) ) :
	function finvision_the_post_meta( $post_format = 'standart' ) {
        $postID = get_the_ID();
...
if ( ! function_exists( 'finvision_the_post_additional_meta' ) ) :
	function finvision_the_post_additional_meta( $post_format = 'standart' ) {
        $postID = get_the_ID();
        if ( 'post' === get_post_type() ) { ?>
            <div class="entry-meta grey small-text content-justify v-spacing v-center">
                <?php if ( function_exists( 'mwt_share_this' ) ) { ?>
                    <?php finvision_share_this( true, 'Share:', 'color-icon border-icon rounded-icon' ); ?>
                <?php } ?>
                <div class="darklinks">
                    <?php if ( comments_open() ) : ?>
                        <a href="<?php comments_link(); ?>">
                            ...
                            <?php
                                comments_number( esc_html__( 'Comments', 'finvision') . ':' . ' 0', esc_html__( 'Comments', 'finvision') . ':' . ' 1', esc_html__( 'Comments', 'finvision') . ':' . ' %' )
                            ?>
                        </a>
                    <?php else: ?>
                        <i class="fa fa-comment highlight" aria-hidden="true"></i>
                    <?php
                        echo esc_html__( 'Comments closed', 'finvision' );
                    endif; ?>
                </div>
            </div>
        <?php }
...
if ( !function_exists( 'finvision_get_icon_v2' ) ) {
    function finvision_get_icon_v2( $icon, $icon_class = '', $icon_container_class = '' ) {
	if ( $icon['type'] !== 'none' ) : ?>
...
if ( ! function_exists( 'finvision_is_woocommerce_page' ) ) :
	function finvision_is_woocommerce_page() {
		$res = false;
