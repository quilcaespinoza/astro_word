...
if ( $preloader_enabled ) : ?>
	<!-- page preloader -->
	<div class="preloader">
		<div class="preloader_image pulse"<?php if ( $preloader_image ) { echo ' style="background-image: url(' . esc_url( $preloader_image['url'] ) . ')"'; } ?>></div>
	</div>
<?php endif; //preloader_enabled ?>

<!-- search modal -->
<div class="modal" tabindex="-1" role="dialog" aria-labelledby="search_modal" id="search_modal">
	<button type="button" class="close" data-dismiss="modal" aria-label="<?php echo esc_attr__( 'Close', 'finvision' ) ?>">
...
<div id="canvas" class="<?php echo esc_attr( $canvas_class ); ?>"<?php echo ( ! empty( $body_background_image ) ) ? ' style="background-image:url(' . esc_url( $body_background_image['url'] ) . ');"' : ''; ?>>
	<div id="box_wrapper" class="<?php echo esc_attr( $box_wrapper_class ); ?>">
		...
		$header = finvision_get_predefined_template_part( 'header' );
		get_template_part( 'template-parts/header/' . esc_attr( $header ) );

        do_action( 'finvision_slider' );

		if ( ! is_front_page() && ! is_404() && !( is_single() && !defined( 'FW' ) ) ) {
			$breadcrumbs = finvision_get_predefined_template_part( 'breadcrumbs' );
        ...
        ?>
        <section class="ds gradient lightened page_content section_404 <?php echo !empty( $background_image ) ? 'background_cover' : ''; ?> section_padding_top_150 section_padding_bottom_150" <?php echo !empty( $background_image ) ? 'style="background-image: url(' . esc_url( $background_image['url'] ) . ')"' : ''; ?>>
        <?php else : ?>
        <section class="<?php echo esc_attr( $main_section_class ); ?> page_content section_padding_top_150 section_padding_bottom_150 columns_padding_30">
		<?php endif; ?>
			<div class="container">
				<div class="row">
					...
					<?php if ( is_home() ) {
						...
						if ( $blog_slider_enabled ) {
							do_action( 'finvision_blog_slider');
						}
					} ?>
                    ...
                    <?php if ( is_post_type_archive( 'give_forms' ) ) {
	                    ...
	                    if ( $donations_slider_enabled ) {
		                    do_action( 'finvision_donations_slider');
	                    }
                    } ?>
